...
IMG_CHANNELS = 1  # Based on black or white or color
SIM_PATH = './DonkeySimLinux/donkey_sim.x86_64'
MODEL_PATH = './new_models/rl1.h5'
...
class DQNAgent:
    ...
    def build_transfer_model_has_batchnorm(self):
        source_model = load_model('./Shashanks_Files/my_model_gs1.h5')
        model = Sequential()
    ...
    def build_transfer_model_has_no_batchnorm(self):
        source_model = load_model('./Shashanks_Files/my_model_gs.h5')
        model = Sequential()
        for layer in source_model.layers[:-3]:
            model.add(layer)
        for layer in model.layers:
    ...
    def build_transfer_model_has_batchnorm_with_canny(self):
        source_model = load_model('./Shashanks_Files/my_model_canny.h5')
        model = Sequential()
        for layer in source_model.layers[:-3]:
            model.add(layer)
        for layer in model.layers:
...
def run_ddqn(args):
    ...
    try:
        ...
        for e in range(EPISODES):
            ...
            s_t = agent.process_image_for_canny(obs)

            # In Keras, need to reshape
            s_t = s_t.reshape(1, s_t.shape[0], s_t.shape[1], IMG_CHANNELS)  # 1*64*64*4
            while not done:
                ...
                s_t1 = agent.process_image_for_canny(next_obs)

                s_t1 = s_t1.reshape(
                    1, s_t1.shape[0], s_t1.shape[1], IMG_CHANNELS)  # 1x64x64x3
