...
class DQNAgent:
    ...
    def __init__(self, state_size, action_space, train=True):
        ...
        self.model = self.build_model_1()
        self.target_model = self.build_model_1()
    ...
    def build_model_1(self):
        source_model = load_model('./Shashanks_Files/my_model.h5')
        model = Sequential()
...
def run_ddqn(args):
    ...
    try:
        ...
        for e in range(EPISODES):
            ...
            s_t = s_t.reshape(
                1, s_t.shape[0], s_t.shape[1], s_t.shape[2])  # 1*80*80*4
            while not done:
                ...
                x_t1 = x_t1.reshape(
                    1, x_t1.shape[0], x_t1.shape[1], 1)  # 1x80x80x1
                s_t1 = np.append(x_t1, s_t[:, :, :, :2], axis=3)  # 1x80x80x4
                # print(s_t1.shape)
                # exit()
