...
class Client():
    def __init__(self,H=None,p=None,i=None,e=None,t=None,s=None,d=None,vision=False):
        ...
        self.debug= False
        self.maxSteps= 100000000  # 50steps/second
        ...
        while True:
            ...
            try:
                ...
                if n_fail < 0:
                    ...
                    tns = ['aalborg', 'alpine-1', 'alpine-2', 'brondehach', 'corkscrew',
                    ...
                    # t = random.choice(tns)  + '*road'
                    t = 'street-1' + '*road'
                    if self.vision is False:
                        os.system('torcs -nofuel -nodamage -nolaptime -tr ' + t +' &')
                    else:
                        os.system('torcs -nofuel -nodamage -nolaptime -vision -tr ' + t +' &')
...
def drive_example(c):
    ...
    obs = C.S.d
    O = make_observaton(obs)
    real_steer = R['steer']
    
    # add noise
    R['steer'] = np.random.normal(R['steer'],0.07)
...
def make_observaton(raw_obs):
    ...
    if vision is False:
        ...
        image_rgb = obs_vision_to_image_rgb(raw_obs[names[10]]).swapaxes(2,1)
        image_rgb = np.ascontiguousarray(image_rgb.transpose(1,2,0))
        image_rgb = cv2.rotate(image_rgb, cv2.ROTATE_90_COUNTERCLOCKWISE)
        # image_rgb = cv2.Canny(image_rgb,100,200)

        return Observation(focus=np.array(raw_obs['focus'], dtype=np.float32)/200.,
                           speedX=np.array(raw_obs['speedX'], dtype=np.float32)/80,
                           speedY=np.array(raw_obs['speedY'], dtype=np.float32)/80,
                           speedZ=np.array(raw_obs['speedZ'], dtype=np.float32)/80,
                           angle=np.array(raw_obs['angle'], dtype=np.float32)/math.pi,
                           opponents=np.array(raw_obs['opponents'], dtype=np.float32)/200.,
...
if __name__ == "__main__":
    # for k in range(1):
    data = []
    C= Client(p=3101,vision=True)
    for step in range(3000,0,-1):
        C.get_servers_input()
        img,ac = drive_example(C)
        data.append((img,ac))
        # print("left" if ac>0 else "right")
        C.respond_to_server()
    C.shutdown()
    for i,dat in enumerate(data):
        arr = dat[0]
        # arr = np.ascontiguousarray(arr.transpose(1,2,0))
        # arr_gs = np.mean(arr, axis=2)

        img = Image.fromarray(arr, 'RGB')
        # img_gs = Image.fromarray(np.uint8(arr_gs)*255, 'L')
        img.save('./images_st/'+str(0)+'_'+str(i)+'_'+str(dat[1])+'.png')
        # img_gs.save('./imgs_gs/'+str(k)+'_'+str(i)+'_'+str(dat[1])+'.png')
